...
final class MetronomeEngine: @unchecked Sendable {
    ...
    /// 상태 소유자가 수명을 마칠 때 오디오 하드웨어와 알림 관찰자를 명시적으로 정리합니다.
    ///
    /// `deinit` 중에는 렌더 콜백이 부분 해제된 self를 기다릴 수 있으므로 호출하지 않고,
    /// 엔진을 온전히 소유한 `MetronomeState`가 해제되기 직전에 호출합니다.
    func shutdown() {
        gateOpen.store(false, ordering: .relaxed)
